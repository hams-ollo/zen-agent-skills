...
- **Prompt-injection paths**, where content a skill is designed to read (a diff, an issue body, a fetched page, a file in the target repo) can redirect the agent's behavior. No skill or rules module here addresses this class yet: it is recorded as held, with its trigger, in the held section of [`.agents/rules/autonomy.md`](.agents/rules/autonomy.md), so a report of one is new ground rather than a broken promise.
