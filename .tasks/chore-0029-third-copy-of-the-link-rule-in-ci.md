...
- [ ] The link rule is authored in exactly one place, and `.github/workflows/checks.yml` calls it
      rather than restating it.
- [ ] The CI step still covers root `*.md`, `.github/**`, and `docs/**`, and still reports the
      document count it checked.
- [ ] Running the docs check locally over the current tree reports 38 documents and 0 broken links,
      matching the CI baseline measured 2026-08-06.
- [ ] A deliberately broken relative link in one of those documents makes the check exit non-zero,
      demonstrated rather than assumed.
- [ ] The `LINK_RE` comment in `.tasks/validate.py` claiming the CI copy is identical is either true
      again or removed.
- [ ] The scaffolded template validator is still self-contained: a freshly scaffolded repository
