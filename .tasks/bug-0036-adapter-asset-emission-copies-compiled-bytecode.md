...
- [ ] A `__pycache__` directory and a `.pyc` file in a fixture skill directory are absent from every
      emitted target, proven by a test that fails against the current code.
- [ ] A `.py` file that is not bytecode, such as the shipped `templates/validate.py`, still emits,
      proven in the same test.
- [ ] The exclusion predicate matches `install.py`'s in both halves, suffix and path part.
