...
Found while closing [`feat-0035`](done/feat-0035-draft-review-depth.md). The blessing went ahead
...
- [ ] `quick_validate.py` has been run across all 20 skills, with the date and the exact result
      recorded in the task closeout.
- [ ] Anything it reports on `review-depth` is fixed, or recorded as accepted with a stated reason.
- [ ] The `ARCHITECTURE.md` sentence names the new date, count, and result, and no longer points at
      this task as outstanding.
- [ ] A decision is recorded either way on whether the check becomes repeatable, with reasoning.
- [ ] `scripts/validate-skills.py` still exits 0 with no new errors or warnings.
