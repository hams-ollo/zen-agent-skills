...
- `.agents/skills/**`, already covered by
  [chore-0036](done/chore-0036-link-check-skill-supporting-files.md).
- Amending `docs/spec/validate-skills.md`. This rule reaches outside `.agents/skills/` for the
  ...
  [chore-0054](done/chore-0054-amend-validate-skills-spec-for-the-supporting-file-rule.md). File it
...
- [ ] A test writes a rules file carrying a link to a file that does not exist and asserts the run
      reports it and exits non-zero.
- [ ] A test writes a rules file carrying a link that resolves above the `.agents/` tree and
      asserts the run reports it as an escape, not merely as resolving.
- [ ] The run reports how many non-skill `.agents/` markdown files it link-checked, and a test
      asserts that number against a fixture tree rather than only that the run passed.
- [ ] Run over the current tree, the new check reports zero errors, and that output is recorded in
      the closeout.
