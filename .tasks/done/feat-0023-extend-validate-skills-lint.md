...
- [x] A `SKILL.md` linking to a nonexistent relative path produces an error and exit 1.
- [x] A `SKILL.md` linking to `../<name>/SKILL.md` for a nonexistent skill produces an error and
      exit 1.
- [x] A `SKILL.md` asserting both draft and shipped status produces a warning and exit 0.
- [x] `http`, `https`, and `mailto` links, and anchors, are not reported.
- [x] A new test covers each of the three checks above, plus the skip cases.
- [x] `python -m unittest discover -s tests -p "test_*.py"` exits 0 with more tests than the current 11.
- [x] `python scripts/validate-skills.py` still exits 0 against the real `.agents/skills/` tree, or,
...
- [x] `docs/spec/validate-skills.md` is byte-for-byte unchanged.
- [x] Standard library only; no new dependency. PEP 8.
