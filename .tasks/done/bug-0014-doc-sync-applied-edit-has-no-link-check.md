...
- [x] `doc-sync` step 5 states the post-edit link obligation and names the mechanism that satisfies
      it, rather than describing it in the abstract.
- [x] The instruction states what to do when the check fails (repair or revert), not only that the
      check happens.
- [x] `doc-sync.conformance.md` `S-011` moves from `Diverged` to `Conformed`, with the new clause
      cited as evidence.
- [x] The unreconciled set in that matrix drops the `to-fix` row and its count is corrected.
- [x] No change to `doc-revise`.

## Implementation note (2026-08-05)

The problem statement's claim that "no clause in either skill instructs a post-edit link check" is
half wrong. [`doc-revise`](../../.agents/skills/doc-revise/SKILL.md) does carry one, under "Verify
links after any structural change": "Whenever you move content, rename a file, or touch references,
do a link pass at the end". The gap is real but narrower than stated: that rule is **conditional**
on a structural change, and correcting a stale prose claim is usually none of the three triggers, so
the composition still drops the obligation for the common `doc-sync` case. `S-011` states it
unconditionally. The new step 5 bullet therefore states the check as unconditional on every applied
edit and says so explicitly, rather than assuming the composed rule fires.
