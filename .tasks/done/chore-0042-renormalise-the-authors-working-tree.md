...
Split from [`chore-0041`](chore-0041-working-tree-crlf-against-the-eol-lf-policy.md), which
...
- [x] Both `Path.write_text` sites pass `newline=""`, verified by grep over `scripts/`.
- [x] `git ls-files --eol | grep -c 'w/crlf'` returns `0` in the author's checkout.
- [x] The renormalise commit shows no content change: `git diff --ignore-all-space` against its
      parent is empty.
- [x] The count is still `0` after `python scripts/run-checks.py` has run.
