...
- [x] New tests cover S-013, S-014, and S-015, each named so the scenario it proves is identifiable.
- [x] With no optional source configured, an open report reflects newly appended records without being
      requested again (S-013, S-014).
- [x] The absence of the optional source produces no error, and the default latency is stated as a
      number in `docs/OBSERVATORY.md` (S-014).
- [x] With the event source active, reported figures are identical to a corpus-only run over the same
      work, asserted by comparing both (S-015).
- [x] Each event records which source it arrived from (S-015).
- [x] The hook exits cleanly and blocks nothing when the server is not running, asserted by a test.
- [x] `git diff` shows no change to `.claude/settings.json`.
- [x] `install.py --with-hooks` places the hook and prints its registration rather than applying it.
- [x] Existing tests still pass, unchanged in intent.

## Decisions

- **The reporting surface gained a second writer to the store, and that changed a stated property.**
  `serve.py` documented itself as writing nothing, and an open report cannot reflect new work unless
  something folds that work in. Confirmed with the author before building. Nothing the harness owns
  is written by either writer, which is what `S-009` actually states.
- **The optional source is a spool file rather than the HTTP POST the task's wording suggested.**
  Rejected because a POST route collides with `feat-0060`'s `S-019` enumeration, where every non-GET
  method is bound to the refusal, and because a socket call inside a live session is the one thing
  this task says must never block. A file append cannot hang.
- **`Stop` rather than `PostToolUse`.** One line per finished turn is the granularity the observatory
  needs; a hook on every tool call would spool twenty lines for one turn's worth of the same hint.
  The opencode adapter exposes no end-of-turn event, so it fires per tool and says so in place.
- **A seam left open deliberately**: `corpus_fingerprint`'s `except OSError` is unreachable on this
  platform, because `pathlib` yields nothing for a missing directory and for a file rather than
  raising. It is kept for the permission-error case other platforms produce, and a mutation removing
  it survives. Recorded so the next reader does not take that surviving mutation for a gap.
...
- [x] The `agent-observatory` conformance matrix is updated for S-013, S-014, and S-015.
