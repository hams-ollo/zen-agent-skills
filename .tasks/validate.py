...
def dependency_cycles(edges):
    ...
    exists, and every edge of a ring satisfies both: `bug-0061` measured two otherwise
    valid tasks naming each other passing `--strict` at exit 0, in this copy and in the
    scaffold template alike. The lifecycle rule is that a task is dispatchable once every
    id it depends on is in done/, so no member of a ring ever becomes dispatchable, and
    the only symptom is a person noticing that a batch has nothing ready.
