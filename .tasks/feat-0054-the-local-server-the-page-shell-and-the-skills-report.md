...
- [ ] `grep -rniE "cdn|unpkg|jsdelivr|googleapis|<script src=\"http" scripts/observatory/ui/` returns
      nothing.
- [ ] The server binds to loopback only, asserted by a test rather than by reading the source.
- [ ] No `package.json`, lockfile, or `node_modules/` is introduced anywhere.
- [ ] `docs/OBSERVATORY.md` states which installed-skill roster `S-002` counts against.
...
- [ ] The `agent-observatory` conformance matrix is updated for S-001 and S-002.
