...
link that is not a link. The batch also hit the `fix-batch` dispatch trap for the second recorded
...
292 tests. That is the finding as much as any single item below. **None of the ten is reachable from
the acceptance command**, because every one of them is a claim in prose that stopped being true, and
no validator reads a skill body for whether it agrees with another skill body.
...
skills a single file, `fix-batch` at 396 lines), and that descriptions average 759 characters against
