...
7. **Run telemetry, reporting, and bounds.** Absorbs Epic B item 13 phase 2. **Split into three parts on 2026-08-28** (`chore-0076`), because one paragraph was holding three items with three different hold states and could not say so. The hold it carried read: "Held behind item 5 for the reason it was never built in the first place, telemetry has no consumer while a human is watching every run, and item 5 is what first produces runs nobody is watching." That reason survives for (c) and for neither of the others, which is the whole reason the split was worth making rather than leaving the paragraph to be re-read charitably each time.
    ...
    - **(d) Project board.** [`docs/spec/sitrep.md`](docs/spec/sitrep.md), approved 2026-09-11: a board for one repository derived from that repository's own records, with an evidence tier on every figure, placed into the person's local sessions when they start. Personal and opt-in, registered at user scope only. It reports what is true about the work and what waits on the person, which (b) does not, and it composes (b) rather than duplicating it: a view across several repositories is meant to live in (b)'s scope selector, as a later amendment to `agent-observatory`. **Approved, and decomposed** into `feat-0066` to `feat-0071`; the readiness gate ([`docs/spec/sitrep.readiness.md`](docs/spec/sitrep.readiness.md)) passed on its second run, after the author settled one contradiction between the spec and the hooks module.
