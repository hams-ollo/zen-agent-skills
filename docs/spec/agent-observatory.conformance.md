...
`feat-0054`'s, `feat-0055`'s, and `feat-0060`'s.

**Twelve of twenty-two scenarios are built.** `feat-0053` scoped the store and the ingester;
...
`S-019` and `S-020`. The remaining ten are `not-built` and that is the correct answer rather
...
The spec carries **22** scenarios, `S-001` to `S-022`. This matrix has **22** rows: **12**
conformed, **0** diverged, **10** not-built. 12 + 0 + 10 = 22, and the arithmetic is stated
rather than the claim, per the incident `feat-0036` recorded and `chore-0033` named.

Nothing in the audited set is unreconciled. The audited set is the twelve scenarios the four
closed tasks declare between them: the six in `feat-0053`'s `scenarios` frontmatter, the two in
`feat-0054`'s, the two in `feat-0055`'s, and the two in `feat-0060`'s. The ten not-built rows were
established by confirming no code implements them, not by inspecting an implementation.
