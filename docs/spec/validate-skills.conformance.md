...
- **audited**: S-001 through S-025, and all four Proposed Surface elements (invocation, what it
  reads, exit code, output format). The arithmetic, written out because a matrix that asserts full
  coverage without it is the failure `spec-conformance` exists to prevent: the spec carries 25
  scenarios, numbered S-001 to S-025 with no gap and none retired, and 4 surface elements, so 25 + 4
  = 29 spec items. This matrix carries 25 rows under `Scenarios` and 4 under `Proposed Surface`, 25 +
  4 = 29 rows. Every spec item was checked. S-022 is numbered after S-021 and placed beside S-013 in
  ...
  subject is the absence of an inbound reference and it is read against nothing above it.
